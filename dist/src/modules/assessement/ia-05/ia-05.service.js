...
class IA05Service {
    static getQuestions(assessmentId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const questions = yield drizzle_1.db.select().from(schema_1.ia05Question).where((0, drizzle_orm_1.eq)(schema_1.ia05Question.assessmentId, assessmentId)).orderBy(schema_1.ia05Question.order);
            return Promise.all(questions.map((q) => __awaiter(this, void 0, void 0, function* () {
                const options = yield drizzle_1.db.select().from(schema_1.questionOption).where((0, drizzle_orm_1.eq)(schema_1.questionOption.questionId, q.id));
                return {
                    id: q.id,
                    order: q.order,
                    question: q.question,
    ...
    static getAnswerKeys(assessmentId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const questions = yield drizzle_1.db.select().from(schema_1.ia05Question).where((0, drizzle_orm_1.eq)(schema_1.ia05Question.assessmentId, assessmentId)).orderBy(schema_1.ia05Question.order);
            return Promise.all(questions.map((q) => __awaiter(this, void 0, void 0, function* () {
                const answer = yield drizzle_1.db.query.questionOption.findFirst({ where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.questionOption.questionId, q.id), (0, drizzle_orm_1.eq)(schema_1.questionOption.isAnswer, true)) });
                return {
                    id: q.id,
                    order: q.order,
                    question: q.question,
    ...
    static getAssesseeAnswers(resultId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const header = yield drizzle_1.db.query.resultIa05Header.findFirst({ where: (0, drizzle_orm_1.eq)(schema_1.resultIa05Header.resultId, resultId) });
            if (!header)
                throw new error_1.NotFoundError('IA05 header');
            const answers = yield drizzle_1.db.select().from(schema_1.resultIa05).where((0, drizzle_orm_1.eq)(schema_1.resultIa05.headerId, header.id));
            const mapped = [];
            for (const row of answers) {
                const option = yield drizzle_1.db.query.questionOption.findFirst({ where: (0, drizzle_orm_1.eq)(schema_1.questionOption.id, row.optionId) });
                if (!option)
                    continue;
                const question = yield drizzle_1.db.query.ia05Question.findFirst({ where: (0, drizzle_orm_1.eq)(schema_1.ia05Question.id, option.questionId) });
                if (!question)
                    continue;
                mapped.push({
                    id: question.id,
    ...
    static sendAssesseeResult(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!existingResult)
                throw new error_1.NotFoundError('Result');
            const header = yield drizzle_1.db.query.resultIa05Header.findFirst({ where: (0, drizzle_orm_1.eq)(schema_1.resultIa05Header.resultId, data.result_id) });
            if (!header)
                throw new error_1.NotFoundError('IA05 header');
            const optionIds = data.answers.map(a => Number(a.option_id));
            const options = optionIds.length ? yield drizzle_1.db.select().from(schema_1.questionOption).where((0, drizzle_orm_1.inArray)(schema_1.questionOption.id, optionIds)) : [];
            if (options.length !== optionIds.length)
                throw new error_1.NotFoundError('Option');
            const existingRows = yield drizzle_1.db.select().from(schema_1.resultIa05).where((0, drizzle_orm_1.eq)(schema_1.resultIa05.headerId, header.id));
            const results = [];
            for (const answer of data.answers) {
                ...
                for (const r of existingRows) {
                    const opt = yield drizzle_1.db.query.questionOption.findFirst({ where: (0, drizzle_orm_1.eq)(schema_1.questionOption.id, r.optionId) });
                    if (opt && opt.questionId === selected.questionId) {
                        existingForQuestion = r.id;
                        break;
                    }
                }
                if (existingForQuestion) {
                    yield drizzle_1.db.update(schema_1.resultIa05).set({ optionId: answer.option_id }).where((0, drizzle_orm_1.eq)(schema_1.resultIa05.id, existingForQuestion));
                    ...
                    yield drizzle_1.db.insert(schema_1.resultIa05).values({ headerId: header.id, optionId: answer.option_id, approved: false });
                    const created = yield drizzle_1.db.query.resultIa05.findFirst({ where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.resultIa05.headerId, header.id), (0, drizzle_orm_1.eq)(schema_1.resultIa05.optionId, answer.option_id)) });
                    if (created)
    ...
    static sendAssessorResult(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const r of data.results) {
                const row = yield drizzle_1.db.query.resultIa05.findFirst({ where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.resultIa05.headerId, header.id), (0, drizzle_orm_1.eq)(schema_1.resultIa05.optionId, r.option_id)) });
                if (row) {
    ...
    static approvedByAssessor(resultId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!header)
                throw new error_1.NotFoundError('IA05 header');
            yield drizzle_1.db.update(schema_1.resultIa05Header).set({ approvedAssessor: true }).where((0, drizzle_orm_1.eq)(schema_1.resultIa05Header.id, header.id));
    ...
    static approvedByAssessee(resultId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!header)
                throw new error_1.NotFoundError('IA05 header');
            yield drizzle_1.db.update(schema_1.resultIa05Header).set({ approvedAssessee: true }).where((0, drizzle_orm_1.eq)(schema_1.resultIa05Header.id, header.id));
            const updated = yield drizzle_1.db.query.resultIa05Header.findFirst({ where: (0, drizzle_orm_1.eq)(schema_1.resultIa05Header.id, header.id) });
            if (!updated)
                throw new error_1.NotFoundError('IA05 header');
            return updated;
    ...
    static getResultDetails(resultId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const header = yield drizzle_1.db.query.resultIa05Header.findFirst({ where: (0, drizzle_orm_1.eq)(schema_1.resultIa05Header.resultId, result.id) });
            if (!header)
