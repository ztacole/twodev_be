...
export const ApprovalService = {
  async approveApl01Document(docId: number, user: JwtPayload): Promise<any> {
    ...
    if (!admin) {
      throw new Error(
        "Hanya admin yang dapat melakukan approval dokumen APL-01",
      );
    }

    await db
      .update(resultDocTable)
      .set({ admin_id: admin.id, approved: true })
      .where(eq(resultDocTable.id, docId));
    const resultDoc = await db.query.resultDoc.findFirst({
      where: eq(resultDocTable.id, docId),
    });
  ...
  async createApprovalRequest(input: {
    ...
    const approverExists = await db.query.admin.findFirst({
      where: eq(adminTable.id, input.approverAdminId),
    });
    if (!approverExists) {
    ...
    try {
      switch (targetTableLower) {
        ...
        case "occupation": {
          const o = await db.query.occupation.findFirst({
            where: eq(occupationTable.id, input.targetId),
          });
          targetName = o?.name ?? null;
          break;
        }
        case "scheme": {
          const s = await db.query.scheme.findFirst({
            where: eq(schemeTable.id, input.targetId),
          });
          targetName = s ? s.code || s.name : null;
          break;
        }
        case "assessment": {
          const a = await db.query.assessment.findFirst({
            where: eq(assessmentTable.id, input.targetId),
          });
          targetName = a?.code ?? null;
          break;
        }
        case "schedule": {
          const sch = await db.query.assessmentSchedule.findFirst({
            where: eq(scheduleTable.id, input.targetId),
          });
          if (sch) {
            const asmt = await db.query.assessment.findFirst({
              where: eq(assessmentTable.id, sch.assessment_id),
            });
            const occ = asmt
              ? await db.query.occupation.findFirst({
                  where: eq(occupationTable.id, asmt.occupation_id),
                })
              : null;
            const fmt = (d: any) =>
              d instanceof Date
    ...
    const autoBackupForBase = availableApproversForBase.find(
      (a) => a.id !== input.approverAdminId && a.id !== requester.id,
    );
    const backupIdForBase =
      autoBackupForBase?.id ?? (requester.can_approve ? requester.id : null);
  ...
  async listApprovalRequests(
    user: JwtPayload,
    scope: "all" | "to-approve" | "requested-by-me" = "all",
  ) {
    const admin = await db.query.admin.findFirst({
      where: eq(adminTable.user_id, user.id),
    });
    ...
    if (scope === "to-approve") {
      return await db.query.approvalRequest.findMany({
        where: (tbl, { and, eq: _eq, or }) =>
          and(
            or(
              _eq(tbl.approver_admin_id, admin.id),
              _eq(tbl.backup_admin_id, admin.id),
            ),
  ...
  async resolveApprovalRequest(input: {
    ...
    const request = await db.query.approvalRequest.findFirst({
      where: eq(approvalRequestTable.id, input.id),
    });
    ...
    if (input.decision === "approved") {
      ...
      const refreshed = await db.query.approvalRequest.findFirst({
        where: eq(approvalRequestTable.id, input.id),
      });
      if (!refreshed)
        ...
        switch (targetTable) {
          ...
          case "occupation": {
            ...
            await db
              ...
              .set(updateFields)
              .where(eq(occupationTable.id, targetId))
              .execute();
            ...
            if (tempFilePath && schemeId && name) {
              ...
              const targetDir = path.join(
                __dirname,
                `../../../public/uploads/occupations/${targetId}_${schemeId}_${cleanName}`,
              );
        ...
        switch (targetTable) {
          case "user":
            {
              ...
              await db
                ...
                .where(eq(userTable.id, targetId))
                .execute();
              const afterDelete = await db.query.user.findFirst({
                where: eq(userTable.id, targetId),
              });
              if (afterDelete) {
          ...
          case "occupation": {
            ...
            if (occupation) {
              ...
              const filePath = path.join(
                __dirname,
                `../../../public/uploads/occupations/${occupation.id}_${occupation.scheme_id}_${cleanName}`,
              );
          ...
          case "scheme": {
            const scheme = await db.query.scheme.findFirst({
              where: eq(schemeTable.id, targetId),
            });
            if (scheme) {
              await db
                .delete(schemeTable)
          ...
          case "schedule": {
            const schedule = await db.query.assessmentSchedule.findFirst({
              where: eq(scheduleTable.id, targetId),
            });
            if (schedule) {
              await db
                .delete(scheduleDetailTable)
                .where(eq(scheduleDetailTable.schedule_id, targetId))
                .execute();
              await db
          ...
          case "assessment": {
            const assessment = await db.query.assessment.findFirst({
              where: eq(assessmentTable.id, targetId),
            });
            if (assessment) {
              const schedules = await db.query.assessmentSchedule.findMany({
                where: eq(scheduleTable.assessment_id, targetId),
              });
              for (const schedule of schedules) {
                await db
                  .delete(scheduleDetailTable)
                  .where(eq(scheduleDetailTable.schedule_id, schedule.id))
                  .execute();
              }
              await db
                .delete(scheduleTable)
          ...
          case "assessee": {
            ...
            if (assessee) {
              ...
              await db
                .delete(resultTable)
                .where(eq(resultTable.assessee_id, targetId))
                .execute();

              // Delete the assessee
              await db
                .delete(assesseeTable)
                .where(eq(assesseeTable.id, targetId))
                .execute();

              // Verify deletion
              const verifyDeletion = await db.query.assessee.findFirst({
                where: eq(assesseeTable.id, targetId),
              });
              if (verifyDeletion) {
                throw new Error(
                  "Gagal menghapus assessee - data masih ada setelah penghapusan",
                );
              }
            }
            break;
    ...
    const updated = await db.query.approvalRequest.findFirst({
      where: eq(approvalRequestTable.id, input.id),
    });
    return updated;
  },

  async createApprovalRequestWithBackup(input: {
    user: JwtPayload;
    ...
    const approvalRequest = await db.query.approvalRequest.findFirst({
      where: eq(approvalRequestTable.id, insertResult[0].insertId),
    });
  ...
  async getAvailableApprovers(): Promise<
    Array<{ id: number; user_id: number; can_approve: boolean }>
  > {
  ...
  async createApprovalRequestWithAutoBackup(input: {
    user: JwtPayload;
    primaryApproverId: number;
    targetTable: string;
    targetId: number;
    ...
    const backupApprover = availableApprovers.find(
      (admin) =>
        admin.id !== input.primaryApproverId && admin.id !== requester.id,
    );
    const backupIdAuto =
      backupApprover?.id ??
      (requester.can_approve && requester.id !== input.primaryApproverId
        ? requester.id
        : null);
