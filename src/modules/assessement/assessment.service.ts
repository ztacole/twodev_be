...
export class AssessmentService {
    ...
    static async assesseeNavigation(schedule_id: number, assessor_id: number, assessee_id: number) {
        ...
        const ak03Header = await db.query.resultAk03Header.findFirst({ where: eq(resultAk03Header.result_id, result.id) });
    ...
    static async assessorNavigation(schedule_id: number, assessor_id: number) {
        ...
        for (const tab of tabs) {
            ...
            const priorityMap: Record<AssessorTab["status"], number> = {
                "Belum Tuntas": 1,
                "Menunggu Asesi": 2,
                "Butuh Persetujuan": 3,
                "Tuntas": 4,
            };
            const statusFromPriority = (p: number): AssessorTab["status"] => {
                if (p <= 1) return "Belum Tuntas";
                if (p === 2) return "Menunggu Asesi";
                if (p === 3) return "Butuh Persetujuan";
                return "Tuntas";
            };
            ...
            for (const result of results) {
                let header: any = null;
                let localStatus: AssessorTab["status"] | null = null;
                ...
                switch (tab.name) {
                    case 'APL-02':
                        ...
                        if (header) {
                            ...
                            if (!finishedApl02 && !a && !b) localStatus = "Menunggu Asesi";
                            else if (finishedApl02 && !a && !b) localStatus = "Butuh Persetujuan";
                            else if (a && !b) localStatus = "Menunggu Asesi";
                            else if (a && b) localStatus = "Tuntas";
                        }
                        break;
                    case 'AK-01':
                        ...
                        if (header) {
                            const a = header.approved_assessor === true;
                            const b = header.approved_assessee === true;
                            if (!a && !b) localStatus = "Belum Tuntas";
                            else if (a && !b) localStatus = "Menunggu Asesi";
                            else if (a && b) localStatus = "Tuntas";
                        }
                        break;
                    ...
                    case 'IA-02':
                        ...
                        if (header) {
                            const a = header.approved_assessor === true;
                            const b = header.approved_assessee === true;
                            if (!a && !b) localStatus = "Butuh Persetujuan";
                            else if (a && !b) localStatus = "Menunggu Asesi";
                            else if (a && b) localStatus = "Tuntas";
                        }
                        break;
                    ...
                    case 'IA-05':
                        ...
                        if (header) {
                            ...
                            if (!ia05Result) localStatus = "Menunggu Asesi";
                            else {
                                const a = header.approved_assessor === true;
                                const b = header.approved_assessee === true;
                                if (!a && !b) localStatus = "Butuh Persetujuan";
                    ...
                    case 'Penilaian':
                        header = await db.query.resultAk05.findFirst({ where: eq(resultAk05Table.result_id, result.id) });
                        if (header) {
                            if (header.approved_assessor !== true) localStatus = "Butuh Persetujuan";
                            else if (header.approved_assessor === true && result.score === -1) localStatus = "Belum Tuntas";
                            else localStatus = "Tuntas";
                        }
                        break;
                }
