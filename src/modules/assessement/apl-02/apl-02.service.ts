...
export class APL02Service {
  ...
  static async getUnitsAPL02(result_id: number): Promise<any[]> {
    ...
    if (!schedule) {
      throw new NotFoundError('Assessment Schedule');
    }
    ...
    const elementsByUc = await Promise.all(unitCompetencies.map(async (uc) => {
      ...
      const results = await db.select().from(apl02RowTable).where(eq(apl02RowTable.result_apl02_id, apl02Header.id));
